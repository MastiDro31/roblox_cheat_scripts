...
local Stamina_toggle = home_tab:CreateToggle({
...
local Toggle_plr_ESP = plrs_ESP_tab:CreateToggle({
    Name = "Toggle ESP",
    CurrentValue = true,
    Flag = "TogglePlrsESP",
    Callback = function(Value)
...
local Killers_Toggle = plrs_ESP_tab:CreateToggle({
    Name = "Toggle Killers",
    CurrentValue = true,
    Flag = "KillersToggle",
    Callback = function()
...
local Killers_fill = plrs_ESP_tab:CreateColorPicker({
    Name = "Killers Fill",
...
local Killers_outline = plrs_ESP_tab:CreateColorPicker({
...
local Survivors_Toggle = plrs_ESP_tab:CreateToggle({
...
local Survivors_fill = plrs_ESP_tab:CreateColorPicker({
    Name = "Survivors Fill",
...
local Survivors_outline = plrs_ESP_tab:CreateColorPicker({
...
plrs_ESP_tab:CreateDivider()
local Spectating_Toggle = plrs_ESP_tab:CreateToggle({
...
local Spectating_fill = plrs_ESP_tab:CreateColorPicker({
...
local Spectating_outline = plrs_ESP_tab:CreateColorPicker({
...
local reload_esp_button = plrs_ESP_tab:CreateButton({
    Name = "Reload ESP",
    Callback = function()
        disable_plrs_esp()
        update_plrs_esp_colors()
...
local min_range_Auto_solver = gens_tab:CreateSlider({
...
local Solve_generator_bind = gens_tab:CreateKeybind({
...
local Toggle_gens_ESP = gens_tab:CreateToggle({
    Name = "Toggle ESP",
    CurrentValue = true,
    Flag = "ToggleGensESP",
    Callback = function(Value)
...
local Gens_fill = gens_tab:CreateColorPicker({
    Name = "Gens Fill",
...
local Gens_activated_fill = gens_tab:CreateColorPicker({
...
local Gens_outline = gens_tab:CreateColorPicker({
    Name = "Gens Outline",
    Color = Color3.fromRGB(128,128,128),
    Flag = "GensOutline",
    Callback = function()
...
local Gens_activated_outline = gens_tab:CreateColorPicker({
    Name = "Gens Outline",
    Color = Color3.fromRGB(128,128,128),
    Flag = "GensOutline",
    Callback = function()
...
local reload_esp_button = gens_tab:CreateButton({
    Name = "Reload ESP",
    Callback = function()
        disable_gens_esp()
        update_gens_esp_colors()
...
function disable_plrs_esp()
    for i_team,team: Folder in pairs(Chars_folder:GetChildren()) do
        for i_char,char: Model in pairs(team:GetChildren()) do
            remove_esp(char)
        end
    end
end
    ...
    for i_team,team: Folder in pairs(Chars_folder:GetChildren()) do
        for i_char,char: Model in pairs(team:GetChildren()) do
            if plr_colors[team.Name] ~= nil then
                add_esp(char, plr_colors[team.Name]["fill"], plr_colors[team.Name]["fill_transparency"], plr_colors[team.Name]["outline"], plr_colors[team.Name]["outline_transparency"])
            end
        end
    end
end
...
function disable_gens_esp()
    ...
    for i=1,30 do
        ...
        if MapModel ~= nil then break end
        wait()
    end
    for i_gen,gen: Model in pairs(MapModel:GetChildren()) do
        if gen.Name ~= "Generator" then continue end
        remove_esp(gen)
    end
end
function enable_gens_esp()
    ...
    for i_gen,gen: Model in pairs(MapModel:GetChildren()) do
        ...
        add_esp(gen, gen_colors["fill"], gen_colors["fill_transparency"], gen_colors["outline"], gen_colors["outline_transparency"])
    end
end
